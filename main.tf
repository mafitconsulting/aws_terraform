...
resource "aws_subnet" "wp_public1_subnet" {
  ...
  # retrieved first available availability zone
...
resource "aws_subnet" "wp_private1_subnet" {
  ...
  # retrieved next available availability zone
  availability_zone = "${data.aws_availability_zones.available.names[0]}"
...
resource "aws_security_group" "wp_private_sg" {
  name        = "wp_private_sg"
  description = "Used for access to instances within the private subnet"
  vpc_id      = "${aws_vpc.wp_vpc.id}"
  ...
  egress {
    ...
    to_port     = 0
    protocol    = "-1"          #allow all protocol
    cidr_blocks = ["0.0.0.0/0"] #  to allow access to the s3 endpoint
  }
}
...
resource "aws_security_group" "wp_rds_sg" {
  name        = "wp_rds_sg"
  description = "Used for accessing the RDS instances"
  vpc_id      = "${aws_vpc.wp_vpc.id}"

  # SQL access from public and private securoity groups
  ingress {
    from_port = 3306
    to_port   = 3306
    protocol  = "tcp"
...
EOD
  ...
  provisioner "local-exec" {
    command = "aws ec2 wait instance-status-ok --instance-ids ${aws_instance.wp_dev.id} --profile MarkF && ansible-playbook -i aws_hosts wordpress.yml"
  }
}
